...
Wenn Sie eine einfache Meldung nur dann ausgeben möchten, wenn der Build außerhalb von Cloud Manager ausgeführt wird, gehen Sie wie folgt vor:
...
Artefakte aus einem passwortgeschützten Maven-Repository sollten mit Vorsicht verwendet werden, da Code, der auf diese Weise bereitgestellt wird, nicht vollständig den Qualitätsprüfungen unterliegt, die von den Cloud Manager-Qualitätsstandards durchgesetzt werden. Adobe empfiehlt außerdem, die Java-Quellen und den gesamten Projektquell-Code zusammen mit der Binärdatei bereitzustellen.
...
Wenn der Build-Vorgang von Cloud Manager gestartet wird, wird das Element `<servers>` in dieser Datei mit der von Cloud Manager bereitgestellten Standarddatei `settings.xml` zusammengeführt. Benutzerdefinierte Server verwenden Server-IDs, die nicht mit `adobe` oder `cloud-manager` beginnen. Solche IDs werden als reserviert betrachtet. Cloud Manager spiegelt nur die Server-IDs wider, die mit einem der angegebenen Präfixe oder der standardmäßigen ID `central` übereinstimmen.

Wenn diese Datei vorhanden ist, wird die Server-ID von innerhalb eines `<repository>` und/oder `<pluginRepository>` Elements in der `pom.xml` referenziert. Diese `<repository>`- und/oder `<pluginRepository>` sind in einem [Cloud Manager-spezifischen Profil enthalten](#activating-maven-profiles-in-cloud-manager) obwohl dies nicht unbedingt erforderlich ist.

Angenommen, das Repository befindet sich auf `https://repository.myco.com/maven2`, der von Cloud Manager verwendete Benutzername ist `cloudmanager` und das Kennwort ist `secretword`.
...
Es empfiehlt sich, den gesamten Quell-Code des Projekts zusammen mit der Binärdatei in einem Maven-Repository bereitzustellen. Dadurch kann das genaue Artefakt neu erstellt werden.
...
* Das [Maven-Versions-Handling](/help/managing-code/maven-project-version.md) ersetzt die Projektversion nur in Produktions-Pipelines. Wenn derselbe Commit sowohl für Entwicklungs- als auch für Produktions-Pipelines verwendet wird und die Entwicklungs-Pipeline zuerst ausgeführt wird, werden die Versionen unverändert in der Staging- und Produktionsumgebung bereitgestellt. In diesem Fall wird jedoch nach wie vor ein Tag erstellt.
